 ...
 * 排序：最新在最上方。
 *
 * NOTE: Changelog entries are kept as historical Chinese-only records.
 * Only the UI chrome (h1, badges, intro, footer) is i18n-localized.
 */
...
const CHANGELOG: ChangelogEntry[] = [
  {
    date: "2026-06-15",
    badge: "improve",
    title: "免費版：所有模板都能完整預覽了",
    body: [
      "以前免費版點付費模板會直接被擋在外面，現在改成「完整預覽」——任何模板都能看到完整的 3D、三視圖、榫卯細節圖、零件圖、材料單與工序，先看清楚值不值得再決定。",
      "預覽時還能自由切換木料，直接比較不同材質的樣子。",
      "方凳、筆筒、木工工作桌三個入門模板維持完整免費客製。其他模板要改成你自己的尺寸、產生報價或列印工程包時再升級即可。",
      "也新增了「把網站加到手機主畫面」的安裝說明頁（/install），用起來跟 App 一樣。",
    ],
  },
  {
    date: "2026-05-26",
  ...
  },
  {
    ...
    title: "正式開賣 26 個家具範本",
    body: [
      "免費版：方凳、筆筒、木工工作桌（功能跟付費一樣，只有模板數量跟尺寸有上限）。",
...
export default async function ChangelogPage({ params }: PageProps) {
  ...
  const badgeStyle: Record<ChangelogEntry["badge"], { label: string; cls: string }> = {
    ...
    improve: { label: t("badgeImprove"), cls: "bg-amber-100 ring-amber-300 text-amber-800" },
    fix: { label: t("badgeFix"), cls: "bg-rose-100 ring-rose-300 text-rose-800" },
    design: { label: t("badgeDesign"), cls: "bg-blue-100 ring-blue-300 text-blue-800" },
  };
  return (
          ...
          {CHANGELOG.map((e) => (
            <li key={`${e.date}-${e.title}`} className="ml-6 sm:ml-8">
              ...
              <div className="flex flex-wrap items-baseline gap-3 mb-2">
                ...
                <span
                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-bold ring-1 ${badgeStyle[e.badge].cls}`}
                >
