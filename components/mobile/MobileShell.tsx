    ...
    <div className="md:hidden min-h-screen bg-zinc-50 pb-24">
      ...
      <DesignFormShell action={formAction} className="px-4 py-3 space-y-4">
        ...
        {/* 工法 + 設計師模式：核心 toggle，放主表單下方。
            dovetail-box 預設一律榫接（鳩尾盒沒有「組裝版」概念），隱藏 toggle
            並用 hidden input 強制送 joineryMode=true。 */}
        {entry.category === "dovetail-box" ? (
        ...
        <div className="rounded-lg bg-white p-3 border border-zinc-200 space-y-2">
          ...
          {props.canUseDesignerMode ? (
            <label className={`flex items-center gap-2 min-h-[36px] px-1 pt-1 text-sm cursor-pointer ${entry.category !== "dovetail-box" ? "border-t border-zinc-100" : ""}`}>
