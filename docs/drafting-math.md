...
- **AV12 零件圖標註優化（09-08 下午，木頭仁：「零件圖太複雜、重複標註」＋「標太少看不出鑽孔位置」）**：
  ...
  **第二輪（審查員 v2 回報後）肩距鏈規則改成：**
  ...
  審查員 v3 結果：三題圖內標註 725→334、疊字對 62→8、出 A4 30→1；55 孔全部量得到位置。剩下（未做）：同尺寸榫頭／dado 的 ×N 合併、
  T1 垂直標籤避開右端榫頭 inline。⚠️ 鄰孔間距鏈（`hSibs`）對腳件也關掉：凳腳「10/392/279/128」變「10/4/219」木匠要加減才能用。
