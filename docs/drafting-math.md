...
- **AY2 抽屜半隱鳩尾**：側板 `dovetail-ends` 7 段（評審表 28 密合部位）、pinDepth＝側板厚 15；面板 18 厚留 3 面皮（官方 C-C 隱藏線在離背面 6 處，
  shape 只有單一 pinDepth 做不到前後不同深度，先照 15）；後板 15 厚＝貫穿。receiver 靠 id `drawer-1-(front|back)`（mortise-csg `isDovetailReceiver`）。
...
- 未做成造型：腳底 3mm 斜角、木螺釘本體（Ø3.5×30 滑條 ×?、Ø2.4×15 底板 ×3；材料表另有 Ø3×25 ×14 圖面未標位置）。
