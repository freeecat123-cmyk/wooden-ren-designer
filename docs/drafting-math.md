...
|---|---|---|
| 六柱圈椅本機 CAD 曲面 / 正投影輪廓 / 網格材積 | `"cad-mesh\|六柱圈椅"` | §A1.1 |
...
#### A1.1 六柱圈椅本機 CAD 試作（2026-09-07）

前端分件修正：原 CAD 側撐 Y≤22 mm 的尖翹／前鼻併入前腳（布林聯集），側撐從 Y=22 的接合面開始，基準最高點為 Z=354，不能再帶 Z=400 的尖端。前腳保留尖端 Z=400。兩件在 Y=22 共用 `apronPoint` 變形，前腳上段接枝逐漸銜接其直立主幹；左右皆須驗證，不能只把線藏起來。四個部件的 mesh、STEP 邊界及投影輪廓一同重新生成。

進階設定共 24 項：原有座凹／腰弧 + `six-slat-chair-options.ts` 的 22 項。全部以 544×460×400 的基準尺寸定義，再由主尺寸仿射縮放。`chairTransform` 共用於 mesh、B-rep 線框及投影曲線；上圈變形以座高 400 為支點，後脚中心線用落地點至新圈接點的線性插值，保持直料。背條間距改變時，上端沿圈的背側中心線移動；數量固定六枝。座板底=400−板厚，橫撐頂=座板底−懸浮間距，兩條座板支承的頂面保持接座板底。榫孔由變形後橫撐中心與腳座標重算，四組盲榫長／厚／寬可調。

薄座板限制凹深，基準底厚至少 14 mm，限制必須顯示 warning。手機 `chairRingHeight` 是圈總高，不可按主輸入的板緣高夾制。所有新參數默认值保留基準形狀；這些是造型試調範圍，尚非承重或刀路核准。

線框使用 STEP B-rep 邊界與曲面四分點的縱向等參數線（wireCurves），不可對簡化網格抽 EdgesGeometry；後者會把曲面三角碎面顯示成斷線。曲線與實體共用 morph/normalize，端圈保留。每個圓曲件為兩個端圈、接縫及三條縱線。

橫撐接腳先提供 4 組盲榫試配，基準榫深 12、斷面 10×28 mm，隨尺寸縮放。這是細腳保留料厚的試配值，未達 §B2 一般盲榫最小長度，不能當承重定稿。後橫撐肩距扣除後腳在高度 328 的兩側半寬：2×(9+6×328/750)，得到 442.752 mm。CAD 榫孔入口採實際局部表面座標，不能吸附到外形包絡面；圈扶手、背條、側撐與座板固定仍待設計。

`cad-mesh` 是部件局部包絡正規化的網格：各軸 -0.5..0.5，依 `visible` 還原。CAD 的 Z-up 交換 Y/Z 到 code 的 Y-up 時，三角面繞序必須反轉。預覽與 3D 匯出共用 `buildShapeGeometry`，兩條 2D 路徑共用 `cadMeshPolygon`；SVG 以 even-odd 多環 path 保留圈內空白與側撐孔。

局部正投影輪廓由原始網格三角面聯集求出，與網格共用變形。輪廓是曲面近似，並非 CNC 刀路；任意斜切截面尚未提供。形狀增寬／加深／升高作仿射變換，保持後腳直線；腰弧僅改六枝背條。

曲件不能以包絡盒算實重：網格材積用三角面帶符號四面體體積和的絕對值，再乘三軸尺寸。材料／裁切包絡只是初期估算，圈扶手彎料、沿紋毛料與榫卯未定稿。本模板只在 development 登錄，不能對外發布。
