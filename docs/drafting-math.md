...
- **AZ4 圖上沒標、範本自訂**：下橫檔榫長 25（B-B 虛線量到 ≈25~26，32 厚腳留 6~7）、45×24 檔的榫高 30、
  30×18 檔的榫厚 10 與榫長 12、鳩尾 7 段 10°、滑條的前後起訖。
- **AZ5 判讀員標為待確認**：A-A 上段最上面那格是 5（本範本採用，被 155 鏈與「三件同高 260」交叉驗證）還是 8.5；
  後角（抽屜側板↔後板）是純螺釘還是鳩尾＋螺釘（範本做螺釘）；材料表的木釘 29 支與評審表 28 部位、圖上數得出 25 支之間的差。
- 測試 `lib/templates/__tests__/cert-b2.test.ts`（10 條，官方數字逐項＋5 個常數的變異測試都會紅）。
