...
| 「零件沒畫出來」是正常還是 bug / 要分視圖看 | `"沒畫出來\|missByView\|hardMissing"` | §A12a |
...
的線段，對「榫頭↔榫眼配對」的零件對量最短距離，> 0.5mm ＝圖上接不上。改三視圖或模板長度後要跑。

> ⚠️ **「約 8~10 分鐘」這個數字已經過期（2026-09-22 更正）。**
> 2026-09-02 首次寫下時是 434 個設計／16,483 個接合×視圖、約 8~10 分鐘，
> 當時就是用「太慢」當理由把它排除在 `npm run audit` 預設鏈外。
> 2026-09-22 在同一台機器實測：**459 個設計／16,987 個接合×視圖，`real 6.59s`**（約 60 倍差距）。
> 也就是**排除它的理由已經不成立**了。
> 真的要接進預設鏈之前，**必須先把衣帽架底爪那 7 條 1.7mm 的例外收進 main**（見下），
> 否則 `npm run audit` 會永遠紅，連 pre-commit hook 都會被擋住，比不接更糟。
...
#### A12a. 「零件沒畫出來」要分視圖判（2026-09-22）

`missing` 這欄原本只印前 60 條、不分視圖。2026-09-22 全量實測：**1260 條，front=0 / side=0 / top=1260**。
也就是目前**全部都在俯視圖**——牙條、橫撐、前緣條整個被檯面／頂板蓋住，俯視圖本來就畫不到它的線，
這是正常的、不是缺陷。所以「印 60 條」等於把這欄變成純噪音：真的漏畫主視圖也會被埋在後面 1200 條裡看不到。

判法收緊成：

| 視圖 | 意義 | 稽核行為 |
|---|---|---|
| `top` | 多半被檯面／頂板遮住 | ⚠️ 只警告（印前 20 條 + 總數） |
| `front` / `side` | 主視圖上這個零件**一條線都沒有** → 客人照圖做不出這個零件 | ❌ 計入 exit code |
| 認不出視圖 | 訊息格式被改掉、分類失效 | ❌ 計入 exit code（退路是擋，不是放行） |

今天 front/side 實測 0，所以這條收緊**不改變現有結果**（唯一的紅仍是衣帽架那 7 條 1.7mm，見上），
但以後真的漏畫主視圖就會紅。

變異測試（三條都跑過）：
1. front 視圖故意丟掉一個零件的線 → 27 條 ❌，exit 1。
2. 視圖判讀 regex 改成不會 match → 90 條全進「無法判讀視圖」且 exit 1。
3. 反向對照（未變異、`ONLY=stool`）：90 條 top 漏畫 → exit 0，不誤紅。
