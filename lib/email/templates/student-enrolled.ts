...
export function studentEnrolledEmail(input: {
  ...
  if (isEn) {
    const subject = "You've been enrolled in the Carpenter Academy student plan — Wooden Ren Blueprint";
    const text = [
      `Hi,`,
      ``,
      `${input.email} has been added to the Carpenter Academy student whitelist.`,
      ...
      `- Ceiling framing simulator`,
      `- Designer Mode (custom sizes down to mm)`,
      ``,
    ...
    const html = htmlShell(
      "You've been enrolled in the Carpenter Academy student plan",
      `<p>Hi,</p>
<p><strong>${escapeHtml(input.email)}</strong> has been added to the Carpenter Academy student whitelist.
Your Wooden Ren Blueprint account is now upgraded to the <strong>Student</strong> plan, valid through
...
<ul style="padding-left:18px;line-height:1.7;color:#374151;font-size:14px">
  ...
  <li>Ceiling framing simulator</li>
  <li>Designer Mode (custom sizes down to mm)</li>
</ul>
