...
export interface LaborDefaults {
  ...
  /** 手動覆寫「交貨工作天」；0 表示不覆寫（沿用 estimatedWorkdays = ceil(工時/8) + bufferDays）。
   *  用於業主想直接給客戶一個固定交期，不想露出內部工時演算法 */
...
export const LABOR_BOUNDS = {
  ...
  deliveryDaysOverride: { min: 0, max: 365, step: 1 },
