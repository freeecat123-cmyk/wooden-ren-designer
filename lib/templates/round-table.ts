...
function buildPedestalRoundTable(p: {
  ...
  const feet: Part[] = [
    ...
    //   右(+X) 0、左(-X) π、後(+Z) -π/2、前(-Z) +π/2
    const rotY =
      isXAxis ? (f.sign > 0 ? 0 : Math.PI)
...
export const roundTable: FurnitureTemplate = (input): FurnitureDesign => {
  ...
  // 桌面厚 ≤25mm 自動通榫（同 autoTenonType 規則）
    ...
    [-1, 1].map((sz) => ({
      ...
      mortises: [
        ...((() => {
          ...
          // Round-table legs aren't rotated (origin only) so part-local == world.
