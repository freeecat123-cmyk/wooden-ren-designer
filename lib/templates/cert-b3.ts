 ...
 *
 * ⛔ **這裡原本寫的「尺寸區塊的部位數＝件數 × 量測尺寸數，可以反推支數」已於 2026-09-10 推翻**
 *    （做第四題那輪把六題評審表橫向排開才發現）。**部位數一律不可以反推件數**，四條反證：
 *      ① 第一題「內部木釘 **30** 部位」＞ 材料表發的 **29** 支木釘（材料表六題共用一份、木釘一律 29 支）。
 *      ② 同一條「**總寬度**」，第一~四題是 **4** 部位、第五、六題是 **3** 部位
 *         （配分都是 6：4×1.5 ＝ 3×2）。量同一條總寬不可能這題 4 個地方、那題 3 個地方。
 *      ③ 第二題「**側板寬度 155**」是一片板的一個尺寸，卻寫 **4** 部位。
 *      ④ 第六題腳柱同樣是 45×32，卻只有 **4** 部位（第一、二、三、五題是 8）。
 *    ⇒ 部位數是**把配分湊成整數的分母**（實測「配分＝部位數 × 每部位扣分」幾乎全部整除，
 *      例外只有官方自己四捨五入的那一格）。它只能當**旁證**，件數一律回工作圖數。
 *
