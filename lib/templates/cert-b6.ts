...
//
// 【第八輪：獨立複查員（opus）打回第七輪三條主張，以下三條經本人自驗後確認要更正】
// ⭐ 更正A：**深度雙標方向搞反了**。第七輪寫「380＝腳架深、360＝桌面板深（桌面板
//    內縮）」——**反了**。自驗（頁20右下角俯視小圖，該圖內部按比例繪製）：中央大
//    矩形 y 4971..5431＝460px、左右兩條窄長條 y 4983..5419＝436px，比值 0.9478，
//    而 360/380＝0.9474，吻合到 0.04%。中央大矩形（頂板）對到 380、兩側窄條
//    （腳架）對到 360。⇒ **380＝桌面板深度（最外緣）、360＝腳架深度，桌面板前後
//    各懸出腳架 10mm**。照第七輪的寫法建模會把頂板做短 20mm、懸出也消失。
// ⭐ 更正B：**「360」量的是腳柱本身的高度，不是上橫檔頂面**。自驗：y=2006.5 那條
//    12px 粗線橫跨 x 1014..1403＝32.9mm（≒腳柱寬 32），同一個 x 範圍的粗線在
//    y=6258.5（地面、也是兩條尺寸線共同下端）再次出現 ⇒ 有一個 32mm 寬的件從地面
//    站到 360。60×21 橫檔（x 1150..1398＝21mm）只是**嵌在這 32mm 範圍之內**。
//    ⇒ **腳柱高 360、不是 370**；370 只屬於頂板頂面。第七輪「360 的上端落在上橫檔
//    頂緣」是把零件認錯，結論方向對但依據要換成這條。
// ⭐ 更正C：**裂口榫「延後理由作廢」下得太早**，第七輪只對了三分之一。自驗兩點：
//    ①`enforceConstructionLimits()`（construction-cuts.ts:233）只要 `constructionLimits`
//    回傳任何 issue，就**把該零件的 construction mortise 整個 filter 掉**並推一則
//    使用者警告「修訂加工未套用…未解決前不可照此製作」；而上/中橫檔的裂口要沿腳柱
//    45mm 厚**整個貫穿**⇒剩餘肉厚 web=0，必然觸發「less than 6mm web」issue（複查員
//    實跑：貫穿45厚 minWeb=0、10 條 issue；側下橫檔貫穿32寬 minWeb=13、0 issue）。
//    ⇒ **側下橫檔的裂口可以用現成機制做，上/中橫檔的不行**，要做就得改共用層。
//    ②`lib/render/geometry.ts` 對 `constructionCutBox` 的參照數＝**0**（自驗 grep），
//    三視圖 silhouette 根本不讀 construction cut，輪廓仍由 `part.shape`/`part.visible`
//    算 ⇒ 就算挖了料，三視圖也不會出現 U 形開口斷面，只有 3D 透視圖會 CSG 挖掉。
//    原本「要動共用渲染層」的顧慮**成立**，不該作廢。
//
// 【第八輪確認仍然成立的部分】
// - 第七輪主張①（頂到 370 的是 18mm 水平板、不是腳柱端面）**成立且更強**：複查員
//   獨立重新校準比例尺（11.822 px/mm，自校非沿用）後量到同樣的 18.02mm，且該帶
//   內部是等距 9.52mm pitch 的木心板芯材符號（同一 pitch 在 A-A/B-B/C-C 三圖一致）；
//   本人另驗線寬＝12px（零件輪廓）vs 尺寸線 4px，排除「兩條延伸線剛好差 18」。
//   複查員三次嘗試推翻（延伸線誤判／跨視圖／跨折斷線）全部失敗並留有紀錄。
// - 第七輪主張②（「評審表沒列桌面板項」推論法零鑑別力）**成立**，複查員另外回
//   page 19 自己量到 cert-b5 的頂板也是 18.03mm 粗線帶，前提可靠。
// - 頂板規格（複查員量、本人未逐項複量）：**430 × 380 × 18**，前後緣各一條約 8mm
//   實木封邊（圖面直接標「8」，不是材料表項次5 的 8.5——那個歸屬是前幾輪自己猜的）。
//   430＝494−2×32（兩支腳柱寬）。頂面 370、底面 352，比腳頂 360 低 8mm ⇒ 那 8mm
//   是頂板底面套住腳頂的溝槽/榫槽，`addConstructionHousing()` 正好做這件事。
//
// 【第八輪開出的兩張獨立工單（都不在本輪動手，避免拿未驗證結構覆蓋已驗證結構）】
// 🔴 工單1：**cert-b5 可能有同一個方向錯誤**。複查員把同一招套回 page 19，量到
//    b5 的「380」尺寸線 x 4509..8995 跟 b5 頂板自己的邊界 x 4505..9004 是同一段
//    ⇒ `cert-b5.ts` 的 `topD: 370`（頂板370/腳架380）很可能也該對調成頂板380/
//    腳架370。那是已走完 13 輪、宣告收尾的模板，要單獨開工單查證，不要順手改。
// 🔴 工單2：**「只有 2 支腳柱在後方」的證據這次變成 5 條互相印證**（正視小圖只有
//    兩條通高立柱且頂/底各一個開放缺口、側視小圖只有一條通高立柱另一端全空、俯視
//    小圖每側 11+21 兩段貫穿 360 深剛好解釋一直沒歸位的 472＝494−2×11、B-B 俯視
//    每側只有一塊 45 深剖切方塊、評審表腳柱部位數 4 而已知 4 腳的 b5 是 8）。
//    第六輪仲裁當時是用「600×92×21.5 只發 2 支、切不出 4 件 60×21」殺掉這個方向，
//    但在「2 腳＋每側 1 支 60×21 側橫檔」的版本下**只需要 2 件，殺不動**。
//    ⇒ 這是目前對 3D 幾何影響最大的未決項，要獨立開一輪專查，不因為證據多就直接改。
//
