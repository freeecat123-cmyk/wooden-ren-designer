...
export const squareStool: FurnitureTemplate = (input): FurnitureDesign => {
  ...
  // 用「牙板中心 Y」當基準算 splay shift——apron 中軸線正好通過腳中軸線
  // top / bot 邊用各自 Y 的腳位置算梯形 scale（top 比中心短、bot 比中心長）
  ...
  const apronSplayX = splayDx * apronCenterShift;     // apron 中心 X 偏移
  const apronSplayZ = splayDz * apronCenterShift;
  const apronSplayXBot = splayDx * apronBotShift;     // bot edge X 偏移
  const apronSplayZBot = splayDz * apronBotShift;
  const apronSplayXTop = splayDx * apronTopShift;     // top edge X 偏移
  ...
  const aprons: Part[] = apronSides.map((s) => {
    ...
    const bevelAngle = isSplayed
      ...
      : 0;
    // 同軸有 splay → 梯形：以中心 Y 為基準，top 比中心短、bot 比中心長
    // → apron 中軸線正好通過腳中軸線
    const trapTopScale =
      s.axis === "x" && splayDx > 0
  ...
  if (withLowerStretcher) {
    ...
    // 下橫撐用「中心 Y」當基準算 splay shift——中軸線通過腳中軸線
    ...
    const lsSplayXBot = splayDx * lsBotShift;     // bot edge
    const lsSplayZBot = splayDz * lsBotShift;
    const lsSplayXTop = splayDx * lsTopShift;     // top edge
