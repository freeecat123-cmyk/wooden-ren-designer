 ...
 *    → **第十三輪：親自查證材料表註 3，解開「426×178×18×3 片未使用」的最後一個疑點；
 *    五金裝配 17 部位維持第十輪的 11/17，這是圖面資訊本身的極限，經使用者拍板接受現況收尾**
 *    → **第十四輪（本輪）：修好抽屜滑條 Ø3×25 導引孔方向 bug（用 mortiseLocalBox +
 *    partMachiningMatrix 世界座標驗算才抓到，跟 cert-b6 同一段程式碼同一個 bug 一起修）**。
 *    詳見下方「最終彙整」。
 *
 ...
 *
 * ✅ **第十四輪：修抽屜滑條 Ø3×25「鎖入腳柱」導引孔方向 bug**（跟 cert-b6 同一輪 review 發現同一
 *    段程式碼有一模一樣的問題，兩邊獨立修好）。用 `mortiseLocalBox` + `partMachiningMatrix` 直接
 *    算世界座標才抓到：左滑條原本從已經貼腳柱那一面鑽、方向卻朝滑條自己內部鑽（25mm 完全搆不到
 *    腳柱），右滑條因為左右鏡射巧合才對——不是設計對稱，兩側是同一顆 `origin.y` 公式套出相反
 *    結果。改成依左右選「貼抽屜、人手伸得進去鎖螺絲」那一面當入榫面，兩側方向對稱、都朝腳柱鑽。
 *    同時修正 `origin.z` 誤用 `E.runnerH/2`（Z 軸慣例置中，該值其實是頂面邊緣，孔會有一半凸出
 *    滑條外）→ 改 0（正中央）。第九輪的「滑條加寬撐到腳柱」幾何判讀本身沒有錯，錯的只是螺釘孔
 *    這個 cosmetic 標記的方向，不影響第九～十三輪任何已驗證的結構判讀。
 *
...
export const certB5: FurnitureTemplate = (input): FurnitureDesign => {
  ...
  if (withDrawer) {
    ...
    for (const sx of [0, 1] as const) {
      ...
      // 第14輪修正：上面那段「local y=0 那一面朝向腳柱」的判斷是錯的——兩側腳柱在
      // world 座標上左右相反，同一顆 origin.y 對兩側會挑到相反的面。用
      // mortiseLocalBox + partMachiningMatrix 實際算世界座標驗證才發現：左滑條原本
      // 從「已經貼腳柱」那面鑽、方向卻是往滑條內部鑽（跟腳柱反方向，25mm 完全鑽
      // 不到腳柱），右滑條反而因為兩側鏡射而剛好對——不是設計對稱，是巧合。
      // 改法：依 sx 選「貼抽屜、人手伸得進去鎖螺絲」那一面當入榫面，鑽的方向才會
      // 朝腳柱去（25mm 螺釘仍搆不到腳柱全部 38mm 厚，跟本專案其他螺釘孔一樣只畫
      // 部分穿透示意，不代表真的鑽穿）。z 原本寫 E.runnerH/2——Z 軸慣例是置中
      // （-半寬~+半寬），E.runnerH/2 其實是頂面邊緣不是正中央，孔會有一半凸出滑條
      // 外，改成 0 才是滑條高度正中央。
