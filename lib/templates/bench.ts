...
export const benchOptions: OptionSpec[] = [
  ...
  { group: "leg", type: "number", key: "legSize", label: "腳粗", defaultValue: 40, min: 20, max: 120, step: 1 },
  { group: "top", type: "number", key: "topThickness", label: "座板厚", defaultValue: 30, min: 12, max: 60, step: 1 },
  ...
  { group: "apron", type: "number", key: "apronWidth", label: "牙條高", defaultValue: 60, min: 30, max: 200, step: 5, help: "長凳常見 50–70；80+ 配薄座板會頭輕腳重" },
  { group: "apron", type: "number", key: "apronOffset", label: "牙條距座板", defaultValue: 0, min: 0, max: 400, step: 5 },
  ...
  { group: "back", type: "number", key: "windsorPostD", label: "Windsor 邊柱直徑", defaultValue: 32, min: 20, max: 60, step: 2, help: "兩支較粗的車旋邊柱直徑", dependsOn: { key: "endSplat", equals: "windsor" } },
  { group: "back", type: "number", key: "windsorSpindleD", label: "Windsor 圓料直徑", defaultValue: 16, min: 10, max: 35, step: 1, help: "中央細圓料直徑", dependsOn: { key: "endSplat", equals: "windsor" } },
  ...
  { group: "back", type: "number", key: "windsorBowBendMm", label: "Windsor 頂橫木彎弧", defaultValue: 40, min: 0, max: 80, step: 5, help: "頂橫木 (bow) 中央向後彎的最大量；0 = 直線", dependsOn: { key: "endSplat", equals: "windsor" } },
  { group: "back", type: "number", key: "windsorTopRailH", label: "Windsor 頂橫木寬度", defaultValue: 45, min: 25, max: 100, step: 5, help: "頂橫木 (bow) 高度（正視看到的寬度）", dependsOn: { key: "endSplat", equals: "windsor" } },
  { group: "back", type: "number", key: "windsorTopRailT", label: "Windsor 頂橫木厚度", defaultValue: 28, min: 15, max: 60, step: 1, help: "頂橫木 (bow) 厚度（側視看到的深度）", dependsOn: { key: "endSplat", equals: "windsor" } },
  { group: "back", type: "number", key: "windsorBackInset", label: "Windsor 椅背距座板背緣", defaultValue: 0, min: 0, max: 150, step: 5, help: "椅背整體（圓料+邊柱+頂橫木）往前推離座板背緣的距離；0 = 齊平", dependsOn: { key: "endSplat", equals: "windsor" } },
  { group: "back", type: "number", key: "windsorEndInset", label: "Windsor 椅背距座板端面", defaultValue: 0, min: 0, max: 200, step: 5, help: "椅背左右兩端往內縮的距離（邊柱外緣 + bow 兩端）；0 = 齊平座板兩端", dependsOn: { key: "endSplat", equals: "windsor" } },
  { group: "back", type: "number", key: "windsorHeight", label: "Windsor 靠背高度", defaultValue: 350, min: 200, max: 700, step: 10, help: "從座板上緣到 bow 頂端的總高度。標準椅背 350、太師椅 450~500、高背椅 600+", dependsOn: { key: "endSplat", equals: "windsor" } },
  { group: "back", type: "number", key: "slatCount", label: "直料根數", defaultValue: 5, min: 3, max: 12, step: 1, dependsOn: { key: "endSplat", equals: "slatted" } },
  { group: "back", type: "number", key: "slatSize", label: "直料粗細", defaultValue: 20, min: 20, max: 100, step: 5, help: "方料截面，width 跟 thickness 都用這值", dependsOn: { key: "endSplat", equals: "slatted" } },
  { group: "back", type: "number", key: "topRailSize", label: "頂橫木粗細", defaultValue: 50, min: 25, max: 100, step: 5, help: "頂橫木高度，thickness 自動配 25mm", dependsOn: { key: "endSplat", equals: "slatted" } },
  { group: "back", type: "number", key: "slatBackInset", label: "直料距背緣", defaultValue: 0, min: 0, max: 80, step: 5, help: "直料背面跟座板背緣的距離，0 = 齊平", dependsOn: { key: "endSplat", equals: "slatted" } },
  { group: "back", type: "number", key: "slatEndInset", label: "直料距端頭", defaultValue: 0, min: 0, max: 200, step: 10, help: "直料兩端往內縮的距離（頂橫木仍跨整條長邊不動），0 = 齊平座板兩端", dependsOn: { key: "endSplat", equals: "slatted" } },
  { group: "back", type: "number", key: "topRailBendMm", label: "頂橫木向後彎弧", defaultValue: 0, min: 0, max: 80, step: 5, help: "頂橫木中央往後（背側）彎的最大量，給人靠著符合腰背曲線。0 = 直線", dependsOn: { key: "endSplat", equals: "slatted" } },
  ...
  { group: "back", type: "number", key: "ladderRailH", label: "橫格條寬度", defaultValue: 60, min: 25, max: 150, step: 5, help: "橫木的高度（正視看到的寬度）", dependsOn: { key: "endSplat", equals: "ladder" } },
  { group: "back", type: "number", key: "ladderRailT", label: "橫格條厚度", defaultValue: 25, min: 15, max: 50, step: 5, help: "橫木的厚度（側視看到的深度）", dependsOn: { key: "endSplat", equals: "ladder" } },
  { group: "back", type: "number", key: "ladderRailGap", label: "橫格條間距", defaultValue: 40, min: 10, max: 200, step: 5, help: "相鄰橫木之間的空隙；橫木從頂橫木往下依序疊", dependsOn: { key: "endSplat", equals: "ladder" } },
  { group: "back", type: "number", key: "ladderRailBendMm", label: "橫格條向後弧", defaultValue: 0, min: 0, max: 80, step: 5, help: "每條橫格條中央往後（背側）彎的最大量，貼合腰背曲線。0 = 直線", dependsOn: { key: "endSplat", equals: "ladder" } },
  { group: "leg", type: "number", key: "legInset", label: "椅腳內縮", defaultValue: 0, min: 0, max: 300, step: 5 },
  { group: "stretcher", type: "number", key: "lowerStretcherHeight", label: "下橫撐 / 層板離地高", defaultValue: 0, min: 0, max: 400, step: 10, help: "設 0 = 自動 (≈ 腳高的 25%)；下橫撐跟座下層板都用這值定位" },
];
...
export const bench: FurnitureTemplate = (input) => {
  ...
  if (endSplat !== "none") {
    ...
    if (endSplat === "low" || endSplat === "high") {
      ...
      const partH = railBotY - seatTop; // 椅背料完整直立高度（座板上緣 → bow 下緣）
