...
export const barStoolOptions: OptionSpec[] = [
  ...
  { group: "leg", type: "number", key: "legSize", label: "椅腳粗", defaultValue: 50, min: 20, max: 80, step: 1, help: "正方腳預設值。若下方寬/厚另填則優先使用。吧檯椅高 750mm 需 ≥ 50mm 才夠穩（比例 1:15）" },
  { group: "leg", type: "number", key: "legWidthOverride", label: "椅腳寬 X", defaultValue: 0, min: 0, max: 120, step: 1, help: "0 = 用「椅腳粗」；填值 = 沿座板長邊 X 的尺寸（可做扁腳）" },
  { group: "leg", type: "number", key: "legDepthOverride", label: "椅腳厚 Z", defaultValue: 0, min: 0, max: 120, step: 1, help: "0 = 用「椅腳粗」；填值 = 沿座板寬邊 Z 的尺寸" },
  { group: "leg", type: "number", key: "legInset", label: "椅腳內縮", defaultValue: 0, min: 0, max: 150, step: 5, help: "椅腳從座板邊緣往內縮的距離（每邊）" },
  ...
  { group: "top", type: "number", key: "seatThickness", label: "座板厚", defaultValue: 28, min: 15, max: 60, step: 1 },
  { group: "top", type: "number", key: "seatCornerR", label: "椅面四角圓角", defaultValue: 0, min: 0, max: 100, step: 2, help: "俯視看，椅面 4 個角的圓弧半徑；0 = 直角，30~50 是常見柔角" },
  ...
  { group: "top", type: "number", key: "seatBendMm", label: "椅面彎曲", defaultValue: 0, min: 0, max: 25, step: 1, help: "整片椅面像彎合板那樣彎曲，中間下凹比較好坐；四角榫眼位置不受影響。>0 會覆蓋鞍形 / 邊緣 profile，但保留四角圓角" },
  legEdgeOption("leg", 1),
  legEdgeStyleOption("leg"),
  stretcherEdgeOption("stretcher", 1),
  stretcherEdgeStyleOption("stretcher"),
  { group: "stretcher", type: "number", key: "footrestHeight", label: "腳踏高", defaultValue: 350, min: 50, max: 700, step: 10, help: "腳踏離地高度。吧檯椅標準＝座面下 400–450mm（座面 750→腳踏 300–350；座面 800→腳踏 350–400）；counter stool 較矮，距座面約 300mm" },
  { group: "apron", type: "number", key: "apronWidth", label: "牙條高", defaultValue: 50, min: 20, max: 150, step: 5 },
  { group: "apron", type: "number", key: "apronThickness", label: "牙條厚", defaultValue: 18, min: 10, max: 40, step: 1, help: "牙條的水平厚度（垂直於座板邊）" },
  { group: "apron", type: "number", key: "apronOffset", label: "牙條距座板", defaultValue: 0, min: 0, max: 300, step: 5, help: "牙條頂緣往下退的距離" },
  { group: "apron", type: "number", key: "apronStaggerMm", label: "牙條錯開", defaultValue: 0, min: 0, max: 60, step: 2, help: "前後牙條（X 軸）相對左右牙條下移量，3D 即時顯示，榫頭整支跟著。0 = 等高（自動上下半榫避免穿模）" },
  ...
  { group: "back", type: "number", key: "backPanelHeight", label: "弧形板高", defaultValue: 180, min: 100, max: 400, step: 10, help: "椅背板的垂直高（圓角矩形板）", dependsOn: { key: "backStyle", equals: "panel" } },
  { group: "back", type: "number", key: "backPanelThickness", label: "弧形板厚", defaultValue: 18, min: 10, max: 30, step: 1, dependsOn: { key: "backStyle", equals: "panel" } },
  { group: "back", type: "number", key: "backPanelCornerR", label: "弧形板圓角", defaultValue: 30, min: 0, max: 100, step: 2, help: "板的四角圓角半徑", dependsOn: { key: "backStyle", equals: "panel" } },
  { group: "back", type: "number", key: "backPanelTopArch", label: "上緣拱起", defaultValue: 0, min: -80, max: 80, step: 2, help: "板上緣中央位移；正值往上拱（拱形頂），負值往下凹（凹弧頂）；0 = 平頂", dependsOn: { key: "backStyle", equals: "panel" } },
  { group: "back", type: "number", key: "backPanelBottomArch", label: "下緣拱起", defaultValue: 0, min: -80, max: 80, step: 2, help: "板下緣中央位移；正值往上拱（D 形/月牙），負值往下延伸（裙擺）；0 = 平底", dependsOn: { key: "backStyle", equals: "panel" } },
  { group: "back", type: "number", key: "backPanelFaceBend", label: "弧形板大面彎曲", defaultValue: 0, min: 0, max: 80, step: 2, help: "板的大面（前/後）凹陷量；0 = 平板，數值越大越彎（建議 10–30 做 lumbar 腰靠）", dependsOn: { key: "backStyle", equals: "panel" } },
  { group: "back", type: "number", key: "backPostDiameter", label: "圓形支撐柱直徑", defaultValue: 25, min: 15, max: 50, step: 1, help: "支撐椅背板的兩支圓形垂直木", dependsOn: { key: "backStyle", equals: "panel" } },
  { group: "back", type: "number", key: "backPanelInset", label: "靠背距椅面後緣", defaultValue: 0, min: 0, max: 200, step: 5, help: "圓柱後緣從椅面後緣往前的距離；0 = 圓柱後緣與椅面後緣對齊", dependsOn: { key: "backStyle", equals: "panel" } },
  ...
  { group: "back", type: "number", key: "backPostFromEdge", label: "圓柱距板端面", defaultValue: 0, min: 0, max: 200, step: 5, help: "圓柱外緣到靠背板左/右端面的距離；0 = 圓柱外緣齊板端面", dependsOn: { key: "backStyle", equals: "panel" } },
  { group: "back", type: "number", key: "backPanelEmbed", label: "靠背卡入圓柱", defaultValue: 6, min: 0, max: 30, step: 1, help: "靠背板嵌入圓柱的深度；接合處從圓柱扣掉同尺寸的平面", dependsOn: { key: "backStyle", equals: "panel" } },
  { group: "back", type: "number", key: "backHeight", label: "椅背高", defaultValue: 200, min: 80, max: 500, step: 10, help: "從座板上緣到椅背頂", dependsOn: { key: "backStyle", notIn: ["none"] } },
  ...
  { group: "back", type: "number", key: "backSlatWidth", label: "直條寬", defaultValue: 40, min: 15, max: 150, step: 5, dependsOn: { key: "backStyle", equals: "slats" } },
  { group: "back", type: "number", key: "backSlatThickness", label: "直條厚", defaultValue: 16, min: 8, max: 40, step: 1, dependsOn: { key: "backStyle", equals: "slats" } },
  { group: "back", type: "number", key: "topRailHeight", label: "頂橫木寬", defaultValue: 0, min: 0, max: 120, step: 5, help: "0 = 自動（椅背高的 1/3，最大 50）；自己填值會優先", dependsOn: { key: "backStyle", notIn: ["none", "panel"] } },
  { group: "back", type: "number", key: "topRailThickness", label: "頂橫木厚", defaultValue: 22, min: 12, max: 50, step: 1, dependsOn: { key: "backStyle", notIn: ["none", "panel"] } },
  { group: "back", type: "number", key: "backRailInsetZ", label: "支撐柱距椅面後緣", defaultValue: 0, min: 0, max: 200, step: 5, help: "椅背支撐柱後緣從椅面後緣往前的距離；0 = 柱後緣與椅面後緣對齊", dependsOn: { key: "backStyle", notIn: ["none", "panel"] } },
  { group: "back", type: "number", key: "backRailInsetX", label: "支撐柱距椅面端面", defaultValue: 0, min: 0, max: 200, step: 5, help: "椅背支撐柱外緣從椅面左/右端面往內的距離；0 = 柱外緣與椅面端面對齊", dependsOn: { key: "backStyle", notIn: ["none", "panel"] } },
  { group: "back", type: "number", key: "backRailPostWidth", label: "支撐柱寬 X", defaultValue: 25, min: 0, max: 120, step: 1, help: "預設 25 比椅腳細，視覺上更分離（rail/slats 椅常見）；填 0 = 跟椅腳粗一樣", dependsOn: { key: "backStyle", notIn: ["none", "panel"] } },
  { group: "back", type: "number", key: "backRailPostThickness", label: "支撐柱厚 Z", defaultValue: 25, min: 0, max: 120, step: 1, help: "預設 25 比椅腳細；填 0 = 跟椅腳厚一樣", dependsOn: { key: "backStyle", notIn: ["none", "panel"] } },
  { group: "stretcher", type: "number", key: "footrestWidth", label: "腳踏寬", defaultValue: 30, min: 20, max: 60, step: 1, help: "腳踏橫撐的垂直高（粗）" },
  { group: "stretcher", type: "number", key: "footrestThickness", label: "腳踏厚", defaultValue: 22, min: 12, max: 40, step: 1, help: "腳踏橫撐的水平厚（深）" },
  { group: "stretcher", type: "number", key: "footrestStaggerMm", label: "下橫撐錯開", defaultValue: 0, min: 0, max: 60, step: 2, help: "左右下橫撐（Z 軸）相對前後下橫撐上移量，3D 即時顯示，榫頭整支跟著。0 = 等高（自動上下半榫避免穿模）" },
];
