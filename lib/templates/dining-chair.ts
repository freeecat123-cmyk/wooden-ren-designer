...
export const diningChairOptions: OptionSpec[] = [
  ...
  { group: "leg", type: "number", key: "legSize", label: "椅腳粗", defaultValue: 35, min: 20, max: 120, step: 1, help: "正方腳預設值。若下方寬/厚另填則優先使用" },
  ...
  { group: "leg", type: "number", key: "legInset", label: "椅腳內縮", defaultValue: 0, min: 0, max: 200, step: 5, help: "椅腳中心離座板邊緣的內縮量。> 0 讓座板外伸、視覺更俐落" },
  ...
  // 座板
  { group: "top", type: "number", key: "seatThickness", label: "座板厚", defaultValue: 25, min: 12, max: 60, step: 1 },
  { group: "top", type: "number", key: "seatHeight", label: "坐高", defaultValue: DINING_CHAIR.seatHeightMm, min: 350, max: 550, step: 10, help: `地面到座板上緣，一般 ${DINING_CHAIR.seatHeightRangeMm[0]}–${DINING_CHAIR.seatHeightRangeMm[1]}（FWW 共識）` },
  ...
  { group: "top", type: "number", key: "seatBendMm", label: "椅面彎曲", defaultValue: 0, min: 0, max: 25, step: 1, help: "整片椅面像彎合板那樣彎曲，中間下凹比較好坐；四角榫眼位置不受影響。>0 會覆蓋鞍形 / 邊緣 profile / waterfall" },
  // 牙條
  { group: "apron", type: "number", key: "apronWidth", label: "牙條高", defaultValue: 60, min: 30, max: 200, step: 5 },
  { group: "apron", type: "number", key: "apronThickness", label: "牙條厚", defaultValue: 20, min: 10, max: 40, step: 1, help: "牙條的水平厚度（垂直於座板邊）" },
  { group: "apron", type: "number", key: "apronOffset", label: "牙條距座板", defaultValue: 0, min: 0, max: 150, step: 5, help: "牙條頂緣往下退的距離；0 = 齊平座板下緣（最常見）。一木連做模式強制 0。", dependsOn: { key: "rearPostMode", equals: "split" } },
  { group: "apron", type: "number", key: "apronStaggerMm", label: "牙條錯開", defaultValue: 0, min: 0, max: 60, step: 2, help: "前後牙條（X 軸）相對左右牙條下移量；0 = 等高（自動上下半榫避免穿模）" },
  ...
  { group: "back", type: "number", key: "slatWidth", label: "直條寬", defaultValue: 50, min: 15, max: 200, step: 5, dependsOn: { key: "backStyle", equals: "slats" } },
  { group: "back", type: "number", key: "backSlatThickness", label: "直條厚", defaultValue: 18, min: 8, max: 40, step: 1, dependsOn: { key: "backStyle", equals: "slats" } },
  ...
  { group: "back", type: "number", key: "curvedSplatWidth", label: "曲面中板寬", defaultValue: 220, min: 100, max: 450, step: 10, help: "曲面中板的水平寬度", dependsOn: { key: "backStyle", equals: "curved-splat" } },
  { group: "back", type: "number", key: "curvedSplatBendMm", label: "曲面中板凹陷", defaultValue: 20, min: -60, max: 60, step: 2, help: "正值往背面凹（貼合背部）；負值往前凸（外凸）；0 = 平板", dependsOn: { key: "backStyle", equals: "curved-splat" } },
  { group: "back", type: "number", key: "backTopRailHeight", label: "椅背頂橫木高", defaultValue: 50, min: 20, max: 180, step: 5 },
  { group: "back", type: "number", key: "backTopRailThickness", label: "椅背頂橫木厚", defaultValue: 22, min: 12, max: 50, step: 1 },
  { group: "back", type: "number", key: "backInsetFromRearMm", label: "椅背距座面後緣", defaultValue: 0, min: 0, max: 200, step: 5, help: "椅背柱外緣往前縮多少；0 = 跟後腳齊（最常見）。> 0 時 slats/splat 會與後牙板脫開（建議改 ladder/windsor）", dependsOn: { key: "rearPostMode", equals: "split" } },
  { group: "back", type: "number", key: "backInsetFromEndMm", label: "椅背距座面端面", defaultValue: 0, min: 0, max: 200, step: 5, help: "椅背柱外緣往內縮多少；0 = 跟後腳齊（最常見）。會同步縮短頂橫木與椅背元件可用寬", dependsOn: { key: "rearPostMode", equals: "split" } },
  backRakeOption("back"),
  // 扶手
  ...
  { group: "back", type: "number", key: "armrestPostWidth", label: "扶手前柱寬 X", defaultValue: 0, min: 0, max: 120, step: 1, help: "0 = 跟椅腳粗一樣", dependsOn: { key: "withArmrest", equals: true } },
  { group: "back", type: "number", key: "armrestPostThickness", label: "扶手前柱厚 Z", defaultValue: 0, min: 0, max: 120, step: 1, help: "0 = 跟椅腳厚一樣", dependsOn: { key: "withArmrest", equals: true } },
  { group: "back", type: "number", key: "armrestPlankWidth", label: "扶手板寬", defaultValue: 50, min: 25, max: 120, step: 5, help: "扶手板的左右寬（手掌平面）", dependsOn: { key: "withArmrest", equals: true } },
  { group: "back", type: "number", key: "armrestPlankThickness", label: "扶手板厚", defaultValue: 22, min: 14, max: 50, step: 1, help: "扶手板的垂直厚（從上方看到的厚度）", dependsOn: { key: "withArmrest", equals: true } },
  // 橫撐
  ...
  { group: "stretcher", type: "number", key: "lowerStretcherHeight", label: "下橫撐離地高", defaultValue: 0, min: 0, max: 400, step: 10, help: "0 = 自動（坐高的 25%）", dependsOn: { key: "stretcherStyle", notIn: ["none"] } },
  { group: "stretcher", type: "number", key: "lowerStretcherWidth", label: "下橫撐寬", defaultValue: 35, min: 20, max: 60, step: 1, help: "下橫撐的垂直高（粗）", dependsOn: { key: "stretcherStyle", notIn: ["none"] } },
  { group: "stretcher", type: "number", key: "lowerStretcherThickness", label: "下橫撐厚", defaultValue: 18, min: 12, max: 40, step: 1, help: "下橫撐的水平厚（深）", dependsOn: { key: "stretcherStyle", notIn: ["none"] } },
  { group: "stretcher", type: "number", key: "lowerStretcherStaggerMm", label: "下橫撐錯開", defaultValue: 0, min: 0, max: 60, step: 2, help: "左右下橫撐（Z 軸）相對前後下橫撐上移量；0 = 等高（自動上下半榫避免穿模）", dependsOn: { key: "stretcherStyle", notIn: ["none"] } },
];
