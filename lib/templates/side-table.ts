...
export const sideTableOptions: OptionSpec[] = [
  ...
  { group: "leg", type: "number", key: "legSize", label: "腳粗", defaultValue: 35, min: 20, max: 120, step: 1 },
  { group: "top", type: "number", key: "topThickness", label: "桌面厚", defaultValue: 25, min: 12, max: 60, step: 1 },
  ...
  { group: "apron", type: "number", key: "apronWidth", label: "牙條高", defaultValue: 60, min: 30, max: 200, step: 5, dependsOn: { key: "withDrawer", equals: false } },
  ...
  { group: "leg", type: "number", key: "legInset", label: "桌腳內縮", defaultValue: 0, min: 0, max: 300, step: 5 },
  { group: "apron", type: "number", key: "apronOffset", label: "牙條距桌面", defaultValue: 0, min: 0, max: 200, step: 5, help: "邊桌總高約 600，5–10 比例適中", dependsOn: { key: "withDrawer", equals: false } },
  apronEdgeOption("apron", 1),
  apronEdgeStyleOption("apron"),
  { group: "stretcher", type: "number", key: "lowerStretcherHeight", label: "下橫撐離地高", defaultValue: 0, min: 0, max: 500, step: 10, dependsOn: { key: "withLowerStretchers", equals: true } },
  // ----- 前緣抽屜（藏雜物 / 床頭物品）-----
  { group: "drawer", type: "checkbox", key: "withDrawer", label: "加抽屜", defaultValue: false, help: "前緣抽屜，掛在前牙板下方" },
  { group: "drawer", type: "number", key: "drawerHeight", label: "抽屜高", defaultValue: 80, min: 30, max: 250, step: 5, help: "抽屜面板高（不含底下橫撐 20mm）；牙板高 = 抽屜高 + 20", dependsOn: { key: "withDrawer", equals: true } },
  { group: "drawer", type: "number", key: "drawerDepth", label: "抽屜深", defaultValue: 0, min: 0, max: 500, step: 10, help: "0 = 自動（桌寬 -80）", dependsOn: { key: "withDrawer", equals: true } },
  { group: "drawer", type: "number", key: "drawerFaceOffset", label: "面板距正面", defaultValue: 0, min: -20, max: 50, step: 1, help: "0 = 跟正面齊平；正值 = 面板凸出；負值 = 面板內縮", dependsOn: { key: "withDrawer", equals: true } },
  ...
  if (drawerCount > 0) {
    ...
    if (apronPart && topPart) {
      ...
      for (const part of design.parts) {
        ...
        if (part.origin.z >= 0) continue;
        part.mortises = part.mortises.filter(
          (m) => !(Math.abs(m.origin.x) > 0.5 && Math.abs(m.origin.z) < 0.5),
        );
      }
      // 4 支腳的 Z 面母榫（接 apron-left/right）跟著公榫朝中心 sideShift。
      // origin.z 從 ±1 改 ±10 讓 zToFace=7.5 比 xToFace=11 小，
      // mortiseLocalBox depthAxis 才不會被 origin.x 偏移翻成 X 軸（origin.z 數值不影響 cz 輸出）
      for (const part of design.parts) {
        if (!part.id.startsWith("leg-")) continue;
        const towardCenter = part.origin.x < 0 ? 1 : -1;
        for (const m of part.mortises) {
          if (Math.abs(m.origin.z) > 0.5 && Math.abs(m.origin.x) < 0.5) {
            m.origin.x += towardCenter * sideShift;
            m.origin.z = m.origin.z > 0 ? 10 : -10;
          }
        }
      }
      ...
      // rail 兩端進前腳：legSize=35>25 → blind-tenon depth=23
      const railTenonType = legPenetratingTenon ? "through-tenon" : autoTenonType(legSize);
      const railTenonStd = standardTenon({
        type: railTenonType,
        childThickness: apronThicknessActual,
        childWidth: RAIL_H,
        motherThickness: legSize,
      });
      const railTenonLen = railTenonStd.length + (railTenonType === "through-tenon" ? 5 : 0);
      design.parts.push({
        ...
        // shape 設 apron-beveled bevelAngle=0 → svg-views useOwnPolygon=true，
        // tenon 用自己 polygon 畫在 rail 中軸（不會 match 到非預期母榫）
        shape: { kind: "apron-beveled", bevelAngle: 0 },
        tenons: [
          { position: "start", type: railTenonType === "through-tenon" ? "through-tenon" : "shouldered-tenon", length: railTenonLen, width: railTenonStd.width, thickness: railTenonStd.thickness },
          { position: "end", type: railTenonType === "through-tenon" ? "through-tenon" : "shouldered-tenon", length: railTenonLen, width: railTenonStd.width, thickness: railTenonStd.thickness },
        ],
        mortises: [],
      });
