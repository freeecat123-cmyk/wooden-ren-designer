...
export function MaterialList({
  ...
  // BOM 合併處理：collect parts.bomMergeWith=X 的子零件，併進 X 的 row
  // 合併 bbox = world AABB union；視為「同一塊木料 stock」（買 17mm 厚板銑邊）
  ...
  const rows = design.parts
    ...
    // 有合併子零件 → 用 bbox union 當合併 stock；否則用 calculateCutDimensions
