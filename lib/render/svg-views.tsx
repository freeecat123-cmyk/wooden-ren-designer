  ...
  return (
    ...
    >
      ...
      {showDimensions && !isolatePartId && (() => {
        ...
        type Tag = { label: string; text: string; cx: number; cy: number; count: number };
        // key = "label|text"，相同尺寸只留一個，count 累加
        ...
        // 從 part.visible.{length,width,thickness} 對應到世界軸取深度。
        // 用 rotation 計算：把本地 +Y 軸（thickness 方向）轉到世界看哪個軸最大，
        // 太繁瑣 → 改用簡單規則：「深度」= 沿世界 Z 軸的尺寸 = 三維中最接近櫃內深的那個。
        // 對標準擺放的層板 / 分隔板 / 抽屜面 / 門板，width = innerD（深度），
        // 直接取 visible.width 即為深度（mm）。
        const pickDepth = (part: Part): number => Math.round(part.visible.width);

        const addTag = (label: string, text: string, part: Part) => {
          ...
          tagMap.set(key, { label, text, cx: c.x, cy: c.y, count: 1 });
        };

        const addFrontTag = (label: string, part: Part) => {
          const [d1, d2, d3] = pickWDH(part);
          addTag(label, `${d1}×${d2}×${d3}`, part);
        };
        const addSideTag = (label: string, part: Part) => {
          addTag(label, `深 ${pickDepth(part)}`, part);
        };
        const addTagForView = (label: string, part: Part) =>
          view === "front" ? addFrontTag(label, part) : addSideTag(label, part);
        ...
        for (const part of renderDesign.parts) {
          ...
          if (isDrawerFront) {
            ...
            if (!hasFace) addTagForView("抽屜", part);
            continue;
          }
          // 層板：櫃類層板從正視圖看得到水平線、從側視圖看得到完整深度
          const isShelf =
            /(^|-)shelf(-\d+)?$/.test(part.id) || /-shelf-/.test(part.id);
          if (isShelf) {
            addTagForView("層板", part);
            continue;
          }
          // 分隔板（抽屜列之間水平分隔 / 立柱 column partition）
          // id pattern：`z?-drawer-divider-?` / `?-col-partition-?` / `?-sub-partition-?`
          const isDivider =
            /-divider(-\d+)?$/.test(part.id) ||
            /-partition(-\d+)?$/.test(part.id);
          if (isDivider) {
            addTagForView("分隔板", part);
            continue;
          }
        }

        if (tagMap.size === 0) return null;
        // 防撞：side view 把同 X 軸（同視覺位置）的多個 tag 沿 Y 排開
        // front view 已用 dedup（同尺寸 ×N）所以多半不撞；side view 把所有
        // shelf/drawer/door 都壓到同一 X 區域，垂直堆疊容易蓋字
        // → 簡單作法：按 cy 排序後檢查間距，太近就往下推
        const tags = Array.from(tagMap.values()).sort((a, b) => a.cy - b.cy);
        if (view === "side") {
          const MIN_GAP = 24; // SVG mm，約一行 fontSize=20 字高
          for (let i = 1; i < tags.length; i++) {
            if (tags[i].cy - tags[i - 1].cy < MIN_GAP) {
              tags[i].cy = tags[i - 1].cy + MIN_GAP;
            }
          }
        }
        return (
