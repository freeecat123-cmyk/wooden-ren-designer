...
export function T2Annotations({
  ...
  /** 2D convex hull (Graham scan / Andrew monotone chain)。回傳 CCW 順序 outline。 */
    ...
    const partEls: React.ReactNode[] = [
      isRoundFeature ? (
        <g key={`${it.kind}-${it.idx}-box`}>
          ...
          // mortise/tenon 有 rotation（splayed 配置）→ 用 convex hull polygon 畫斜邊
          // 而非 AABB rect。fallback: 無 rotation 維持 rect 不變。
