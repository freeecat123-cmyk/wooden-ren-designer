...
## ⛔ 「看得到的東西」不准只用截圖驗（2026-09-05 訂）

木頭仁連續三次回報同一個功能的層數不對（2 像 3、4 像 5、3 又多一條），
**每次原因都不同**，而我每次只截一個角度、一端就說修好了 → 換個角度又錯。

規則：改「使用者用看的」的東西（3D 木紋、分件線、孔、接縫）時，
1. **先寫機器可以數的檢查**（數資料、數 SVG 元素），不要用眼睛數截圖；
   例：`lib/render/__tests__/panel-lines.test.ts` 直接數三視圖裡的分件線、
   以及把著色器產生的 GLSL 抓出來驗守衛還在。
2. 截圖只用來「發現」問題，不能用來「證明」修好了。
3. 一定要驗**兩端 / 兩個視角**：工作桌那次的兇手就藏在鉗那端。
4. 每加一條檢查都要做變異測試（故意改壞，確認它會紅）。
