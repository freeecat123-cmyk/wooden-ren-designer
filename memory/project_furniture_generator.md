...
## 2026-09-07 native six-slat chair correction (local only)
- Correct active application for https://designer.woodenren.com is C:/Users/freee/Desktop/wooden-ren-designer. The earlier wooden-ren-designer-deploy experiment on port 3017 was an isolated prototype and was rejected; do not substitute it for the actual app.
- Native development-only catalog entry six-slat-chair now uses the existing design page, mobile shell, material list, drawings, cut-plan/quote/print links and export geometry. Preview: http://localhost:3000/zh-TW/design/six-slat-chair . User explicitly says do not publish; no commit, push or deployment performed.
- Source lib/templates/six-slat-chair.ts and assets/six-slat-chair.json: 20 CAD-derived mesh parts, six curved slats, continuous tapered rear legs, forward bowed front posts, solid scooped seat. CAD coordinate Z-up is converted to app Y-up. Shared cad-mesh routes preserve outline concavity and apron holes in views. Actual mesh volume fixes weight estimation.
- Seat width/depth/rim height plus scoop and lumbar settings; dimensions scale prototype geometry. Manufacturing stock and quote are envelope estimates; bending stock, grain and joinery remain unfinished. No production-ready CNC claim. Existing auth remains in force. STEP source is chair_cad/output_v3/chair_v3.step in the original Documents workspace; changing app parameters does not regenerate that STEP.
- Validation: full 1518-test suite passed before final mobile labels; complete audit and production build passed locally. Final native 8 tests passed, including all 3 export modes and shape outline route. Actual iPhone 13 Playwright interaction changed width 550 and lumbar 25 with URL sync, no covered control and no page errors. Desktop changed width 560 and verified original template names and feature links.
- Pre-existing annotation.tsx and scripts/_probe-dog.ts, scripts/_probe-slab.ts were preserved.

- Wireframe incident follow-up: port 3000 was not listening; restarted Next dev (session 32888). Browser tab title confirms connection refused. Standalone Edge desktop and iPhone 13 both rendered wireframe without console/page errors; repeated on/off worked. No rendering code changed because crash not reproduced. In-app Browser runtime could not operate the existing error-page tab due to its data-URL policy; user needs refresh. Do not claim a proven wireframe root-cause fix.

- 2026-09-07 screenshot clarified wireframe bug: decimated CAD triangle feature edges produced fragmented round surfaces. Native wireCurves now come from STEP B-rep edges + 3 longitudinal surface isolines (6 curves per ring/post/slat), all sharing template morph. Added four provisional 12x10x28 mm rail-to-leg blind tenons and matching mortises; rear shoulder span 442.752 at baseline. True CAD mortise X-entry replaces AABB snapping in cut/render and joint matching. Round-member/apron/seat joints are still unfinished; UI explicitly says so. Tests 1521 passed, full audit passed. User tab 2 successfully verified wireframe and red tenons in actual in-app browser. Never deploy without user authorization.

- 2026-09-07 advanced controls: added 22 numeric options (24 total) in six-slat-chair-options.ts, shared chairTransform updates mesh/wire/projection outlines: ring rise/width/depth/section, back spread/size/rake/lumbar peak, straight tapered rear and front leg sections, rear footprint, post bow, seat thickness/offset/reveal, apron section/curve, four trial tenon sizes. Keeps six slats. Main dimensions scale reference 544x460x400 values. Safe hollow retains 14mm baseline bottom and warns. MobileOptionField excludes chairRingHeight from seat-rim max clamp. EN metadata included. Full tests 1545 passed, audit and local build passed; standalone tsc still has 3 existing workbench test errors. Actual in-app mobile ring slider changed URL and rear leg/ring geometry; restoring original 800/ lumbar29. No deploy. Manufacturing joints remain partial/provisional.

- 2026-09-07 front apron nose ownership: user says projecting nose belongs to front leg. correct_front_partition.py cuts source apron CAD Y<=22 and unions it into matching front leg, for both sides; four mesh/outlines/edge assets regenerated. Apron max source Z now354, front leg tip400. Shared apronPoint transformation gives identical interface atY22 with changed leg size/apron curvature. Added regression for source ownership and current-user parameter seam. Tests1546 passed/full audit passed; current URL parameters preserved. Corrected static STEP at chair_cad/output_v3/chair_front_partition.step (baseline, not current tuned params). Native generation scripts must retain partition correction on any future regeneration.
